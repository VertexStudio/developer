    ...
    ) -> std::pin::Pin<Box<dyn Future<Output = rig::completion::ToolDefinition> + Send + Sync + '_>>
    {
        Box::pin(std::future::ready(rig::completion::ToolDefinition {
            ...
            description: self
                ...
                .unwrap_or_default()
                .to_string(),
            parameters: self.fix_schema_for_openai(),
        }))
    }
...
impl McpToolAdaptor {
    /// Fix MCP schema to be OpenAI-compatible
    fn fix_schema_for_openai(&self) -> serde_json::Value {
        let mut schema = self.tool.schema_as_json_value();

        // Ensure the schema is an object with a properties field
        if let Some(obj) = schema.as_object_mut() {
            // If properties field is missing, add an empty one
            ...
            // If schema is not an object, create a proper one
